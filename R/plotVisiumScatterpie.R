plotVisium_scatterpie <- function(spe, spots = TRUE, annotate = NULL, image = TRUE,
  ...
  p <- ggplot(plt_df, aes(x = pxl_col_in_hires, y = pxl_row_in_hires)) + images +
    list(guides(fill = guide_legend(title = "Cell Type", ncol = 1, order = 1,
                                    override.aes = list(col = NA, size = guide.pt.size)),
                color = "none"),
         geom_scatterpie(aes(x = x, y = y, r = radius),
